...
export default function TopBarUploadAudio() {
  ...
  return (
    <header className="relative w-full min-h-[80px] sm:min-h-[120px] md:min-h-[160px] lg:min-h-[200px] xl:min-h-[232px] flex flex-col py-2 sm:py-3 md:py-4 bg-linear-to-r from-[#65aaff]/15 via-[#d9d9d90a] to-[#0E0E0F] border-b border-[#383838] overflow-hidden">
      {/* gradient overlay */}
          ...
          <div className="hidden lg:flex xl:hidden flex-row flex-wrap gap-2 max-w-full">
            {steps.map((step, idx) => (
              <button
                ...
                onClick={() => handleStepClick(idx)}
                className={`
                  px-3 py-2 rounded-full  font-sans border border-[#383838]
                  transition-shadow duration-500 ease-in-out focus:outline-none
                  flex-shrink-0
                  ${
                    activeStep === idx
                      ? "bg-[#007d49] text-white shadow-lg"
